...
### תיקוני באגים 🐛
- **העברת דגים — "אין דגים באקווריום זה"**:
  - תוקן: רשימת הדגים לא נטענה מחדש בכל פתיחה של חלון ההעברה, רק בפעם הראשונה
  - כעת הדגים נטענים מחדש מ-Firestore בכל פתיחה של חלון ההעברה
- **העברת דגים — דגים שנקלטו (reception fish) לא הופיעו**:
  - `getFishInAquarium` חיפש רק ב-`farmFish` ולא ב-`fish_instances`
  - כעת מוחזרים דגים משני המקורות; `transferFish` מטפל בשניהם נכון
- **z-index של חלון ההעברה כשנפתח מתוך חלון האקווריום**:
  - חלון ההעברה (z-1000) הופיע מאחורי חלון האקווריום (z-1001)
  - תוקן עם prop `zIndex` — מועבר כ-1002 כשנפתח מתוך AquariumFishModal
...
### תכונות חדשות ✨
- **כפתור העברה ישיר בכרטיסיית הדג**:
  - כפתור "🔄 העבר" נוסף לכל כרטיסיית דג בתוך חלון ניהול דגים באקווריום
  - פותח את מודל ההעברה עם האקווריום הנוכחי כמקור — ניתן לבחור כמות ואקווריום יעד ולבצע העברה מיידית
  - עובד גם לדגים מקומיים וגם לדגים שנקלטו ממשלוח
...
### תיקוני באגים 🐛
- **ריקון אקווריום "תמותה כוללת" — דגים נשארו במלאי**:
  - תוקן: כשריקנו אקווריום בסיבת "תמותה כוללת", האקווריום קיבל סטטוס ריק אבל הדגים נשארו עם כמות > 0 ועם aquariumId ← עדיין הופיעו במלאי ובשם האקווריום
  - הסיבה: מסלול "תמותה" רשם את אירוע התמותה אך שכח לאפס את הכמויות ולנקות את ה-aquariumId (כפי שמסלולי "אריזה" ו"מחיקה" כבר עשו)
  - כעת גם "תמותה כוללת" מאפסת כמויות ומנקה aquariumId לאחר רישום התמותה
...
### תיקוני באגים 🐛
- **מחירון — דגים מקומיים מציגים כמות 0 למרות מלאי קיים**:
  - תוקן: דגים שמנוהלים דרך מסך האקווריומים (farmFish) הציגו כמות 0 במחירון
  - הסיבה: המחירון אתחל את כמות כל דג מהקטלוג ל-0 במקום להשתמש בשדה quantity הקיים
  - כעת, כמות הדגים המקומיים נקראת ישירות מהקטלוג; דגי יבוא (fish_instances) מוסיפים מעל
...
### תיקוני באגים 🐛
- **מחירון — מיקום חסר לדגים עם כתובת**:
  - תוקן: דגים שנמצאים באקווריום (לפי קטלוג farmFish) הציגו "-" כמיקום במחירון
  - הסיבה: המחירון ביטל את ה-aquariumId של רשומת הקטלוג ולא השתמש בה כגיבוי כשלרשומת fish_instances אין אקווריום
  - כעת, אם ל-fish_instance אין aquariumId, המחירון יציג את המיקום מרשומת הקטלוג
...
### תיקוני באגים 🐛
- **מחירון — הפרדת "לא במלאי" מ"ללא מחיר"**:
  - דגים ללא מחיר שאינם במלאי הועברו לקטגוריה נפרדת "🚫 לא במלאי" בתחתית הרשימה
  - קטגוריית "⚠️ ללא מחיר - יש לעדכן" מציגה עכשיו רק דגים שיש בהם מלאי ויש לעדכן מחיר
...
### תכונות חדשות ✨
- **חיפוש דגים באקווריומים**:
  - שורת חיפוש חדשה במסך בחירת האזור — הקלד שם דג וראה מיד באיזה חדר ואקווריום הוא נמצא
  - תוצאות החיפוש מציגות חדר + מספר אקווריום + שם הדג
  - לחיצה על תוצאה פותחת את ניהול הדגים לאקווריום הזה ישירות
...
### שיפורים 🔧
- **הזמנות — הצג חדר + לחצן ריקון אקווריום מהיר**:
  - שם החדר מוצג כעת לצד מספר האקווריום גם להזמנות ישנות (נטען בזמן ריצה מנתוני האקווריום)
  - לחיצה על מיקום הדג (📍 חדר #18) פותחת את חלון ריקון האקווריום ישירות מרשימת האריזה
  - לאחר ריקון האקווריום — חוזרים אוטומטית לרשימת האריזה
...
### שיפורים 🔧
- **פורטל הזמנות — הצג כל דגים במלאי**:
  - כעת כל הדגים במלאי מוצגים בפורטל ההזמנות, גם אם אין להם מחיר מעודכן
  - דגים ללא מחיר מציגים "מחיר לא עודכן" במקום להיות מוסתרים
...
### שיפורים 🔧
- **הזמנות — מיקום דג ורוקון אקווריום**:
  - מיקום הדג (חדר + מספר אקווריום) מוצג כעת ישירות בשורת הדג (📍 עליון #18)
  - תיבת סימון "רוקן" מוצגת לכל דג עם אקווריום משויך — כשמסמנים, האקווריום יסומן כריק בסיום ההזמנה
  - נתוני החדר נשמרים כעת בהזמנה בעת יצירתה מהפורטל החיצוני
...
### תיקוני באגים 🐛
- **מחירון דגים — דגים ממשלוח לא הופיעו**:
  - תוקן: דגים שיובאו ישירות דרך "יבוא משלוח" (ללא אקווריום משויך) סוננו בטעות מהמחירון
  - הפילטר עכשיו מסנן רק מופעים שה-aquariumId שלהם מצביע על אקווריום שנמחק — לא על מופעים ללא אקווריום
- **מחירון דגים — מיון לפי שם עברי**:
  - הרשימה ממוינת כעת לפי שם נפוץ (עברי) במקום שם מדעי
...
### שיפורים 🔧
- **יבוא משלוח מקומי — תמיכה מלאה ב-PDF**:
  - ממשק יבוא המשלוח תורגם לעברית (RTL)
  - תוקן: יבוא היה חסום אם אפילו שורה אחת שגויה — כעת שורות שגויות מדולגות ושאר הפריטים מיובאים
  - תמיכה ב-PDF כבר הייתה קיימת דרך Claude AI (PDF, אקסל, CSV, תמונה, טקסט)
...
### תיקוני באגים 🐛
- **הזמנות לא הופיעו מהפורטל החיצוני**:
  - תוקן: שאילתת `getOrders` משתמשת ב-`where('farmId')` יחד עם `orderBy('createdAt')` — שאילתה מורכבת שדורשת אינדקס מורכב ב-Firestore
  - הוסף אינדקס חסר ל-`firestore.indexes.json` והופץ ל-Firestore
  - הזמנות שנוצרו בפורטל החיצוני יופיעו כעת כראוי בתוך האפליקציה
...
### שיפורי UX 🖱️
- **דף אקווריומים - לחיצה על שורה פותחת ניהול דגים**:
  - לחיצה על שורת אקווריום פותחת כעת ישירות את מסך ניהול הדגים (הפעולה הנפוצה)
  - כפתור "✏️ ערוך אקווריום" (אפור) מאפשר גישה לעריכת נתוני האקווריום (פעולה נדירה)
...
### תיקוני באגים 🐛
- **מחירון דגים - שם שגוי לדג עם שם מדעי זהה**:
  - תוקן: דגים עם אותו שם מדעי אך שם נפוץ שונה (לדוגמה: "גופי קוי קוברה" ו"גופי קוי טוקסידו") הופיעו באותה שורה במחירון - והשם הנפוץ הישן (מהקטלוג) דרס את השם החדש (מהמשלוח)
  - פתרון: המפתח למיזוג רשומות במחירון כולל כעת גם את השם הנפוץ, כך שכל שם נפוץ שונה מקבל שורה נפרדת
